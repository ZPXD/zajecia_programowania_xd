...
def flaga():
	...
	if len(os.listdir("static/flag_image")) < 10:
		xd = 11
	ta_flaga = os.path.join(app.config['UPLOAD_FOLDER'], 'flag_image' , 'Polska_Flaga__{}.jpg'.format(xd)) #poprawiony folder i nazwy plikow
...
def gather_heroes():
	...
	heroes = [
 		'Mikołaj Kopernik', 
 		'Rotmistrz Pilecki',
 		'Maria Skłodowska-Curie', #tutaj program przyjmuje polskie nazwy wiec wszystkie tytuly powinny byc z polskimi znakami a maria ma dwa nazwiska
 		'Fryderyk Chopin',
	...
	for hero in heroes:
		if hero not in saved_heroes:
			...
			info_intro = wikipedia.summary(hero,sentences=1) #najprostsza metoda uzyskania 1 akapitu
			...
			# Save all.
			with open('saved_heroes/'+hero+".hero", "w+", encoding="utf-8") as f:   #generalnie nazwy ziomow sa z polskimi znakami i zapisywanie ich do plikow rodzi problemy dlatego dobrze z encodowac na zapisie i odczycie
				f.write(hero + '\n')
				f.write('\n') #str(n_photos) + '\n')
				f.write(info_intro + '\n')
				f.write(url)
	...
	heroes = [] # tutaj powinienen byc slownik?????
	for hero_file in os.listdir('saved_heroes'):
		hero = {}
		some_info = open('saved_heroes/'+hero_file,encoding="utf-8").readlines()
		...
		hero_quotes = open('hero_think/' + hero['name'][:-1] + ".hero",encoding="utf-8").readlines() #tutaj drugi encoding
...
def hero_think(name): #tutaj funkcja jest gitarka
	url_name = name.replace(' ', '_')
	url = 'https://pl.wikiquote.org/wiki/{}'.format(url_name)
	hero_wikiquotes = requests.get(url)
	with open('hero_think/'+name+".hero", "w+", encoding="utf-8") as f:
...
if __name__ == "__main__":
	create_folders() #tutaj przesunalem funkcje z @flaga do maina bo bez sensu
	app.run(debug=True)
