...
def form_b():
  ...
  if form.validate_on_submit():
        ...
        test = form.tekst.data
        napoj = form.napoj.data
        klik = form.klik.data

        string = '{}\n{}\n{}\n\n'.format(test, napoj, klik)
        save_data(string)
...
class F2(FlaskForm):
    there_options = [('w','woda'), ('p', 'piwo'), ('h','herbata'), ('k','kawa'), ('c','cola')]

    tekst = StringField('siema, jak się nazywacz:', validators=[DataRequired()])
    napoj = SelectField("wybierz swój ulubiony napój", choices=there_options)
    klik = BooleanField('mam nadzieję, że nie palisz papierosów')

    b2= SubmitField("wyślij")
